...
while true; do
    ...
    if [ ! -f linux_x86_64/cm ]; then
        echo "  linux_x86_64/cm not found. Waiting for Dropbox sync..."
        echo ""
        continue
    fi

    if [ linux_x86_64/cm -nt CmVersion.h ]; then
        echo "  Linux binary verified (newer than CmVersion.h)"
        break
    else
        echo "  Linux binary is older than CmVersion.h. Not synced yet."
        echo ""
    fi
done
