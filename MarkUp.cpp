...
MarkUp::colorizeText( CxString fullText, CxString visibleText )
{
    ...
    if (_currentSyntax != NULL) {
        ...
        if (_currentSyntax->types != NULL && typeColor.length()) {
            visibleText = colorizeKeywords(visibleText, _currentSyntax->types, typeColor, resetColor);
        }
        if (_currentSyntax->keywords != NULL && keywordColor.length()) {
            visibleText = colorizeKeywords(visibleText, _currentSyntax->keywords, keywordColor, resetColor);
        }
        if (_currentSyntax->constants != NULL) {
            CxString constColor = constantColor.length() ? constantColor : keywordColor;
            if (constColor.length()) {
                visibleText = colorizeKeywords(visibleText, _currentSyntax->constants, constColor, resetColor);
            }
        }
    } else {
        // Fall back to original C++ hardcoded keywords
        if (typeColor.length()) {
